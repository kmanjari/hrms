...
(function (q, r) {
    var o = function (d) {
        var k = function (a, b) {
            var c = this;
            if (!this instanceof k)alert("FixedColumns warning: FixedColumns must be initialised with the 'new' keyword."); else {
        ...
        k.prototype = {
            ...
            fnRecalculateHeight: function (a) {
                ...
                var b =
                    this;
                if ("function" != typeof this.s.dt.oInstance.fnVersionCheck || !0 !== this.s.dt.oInstance.fnVersionCheck("1.8.0"))alert("FixedColumns " + k.VERSION + " required DataTables 1.8.0 or later. Please upgrade your DataTables installation"); else if ("" === this.s.dt.oScroll.sX)this.s.dt.oInstance.oApi._fnLog(this.s.dt, 1, "FixedColumns is not needed (no x-scrolling in DataTables enabled), so no action will be taken. Use 'FixedHeader' for column fixing when scrolling is not enabled"); else {
                    ...
                    if (0 < b.s.iRightColumns)d(b.dom.grid.right.liner).on("mouseover.DTFC touchstart.DTFC",
                        function () {
                            c = "right"
                        }).on("scroll.DTFC", function () {
                ...
                for (var c = [], g = [], f = [], e = 0, h = a.length; e < h; e++) {
                    ...
                    for (var j = 0, n = this.s.iTableColumns; j < n; j++)if (-1 !== d.inArray(j, b)) {
                ...
                if (g) {
                    ...
                    j.empty();
                    e = 0;
                    for (h = m.length; e < h; e++)j[0].appendChild(m[e].nTr);
                    ...
                    for (h = m.length; e < h; e++) {
                        i = 0;
                        for (j = m[e].length; i < j; i++)p[e][i].cell.className = m[e][i].cell.className, d("span.DataTables_sort_icon", p[e][i].cell).each(function () {
                ...
                if (0 < this.s.dt.aiDisplay.length) {
                    h =
                        d(">thead>tr", a.body)[0];
                    for (k = 0; k < c.length; k++)n = c[k], l = d(this.s.dt.aoColumns[n].nTh).clone(!0)[0], l.innerHTML = "", j = l.style, j.paddingTop = "0", j.paddingBottom = "0", j.borderTopWidth = "0", j.borderBottomWidth = "0", j.height = 0, j.width = f.s.aiInnerWidths[n] + "px", h.appendChild(l);
                ...
                if (null !== this.s.dt.nTFoot) {
                    if (g) {
                        ...
                        b.empty();
                        e = 0;
                        for (h = m.length; e < h; e++)b[0].appendChild(m[e].nTr);
                        this.s.dt.oApi._fnDrawHead(this.s.dt, m, !0)
                    } else {
                        m = this._fnCopyLayout(this.s.dt.aoFooter, c);
                        b = [];
                        ...
                        for (h = m.length; e < h; e++) {
                            i = 0;
                            for (j = m[e].length; i < j; i++)b[e][i].cell.className = m[e][i].cell.className
                        }
                    }
                ...
                null !== f.s.dt.nTFoot && (b = this.s.dt.oApi._fnGetUniqueThs(this.s.dt, d(">tfoot", a.footer)[0]), d(b).each(function (a) {
                    n = c[a];
                    this.style.width = f.s.aiInnerWidths[n] + "px"
                }))
